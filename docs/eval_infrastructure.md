...
**Cross-Scientist caveat:** if multiple Scientists contribute labels, `labeled_by` is preserved. Disagreements (same concern labeled tp by one, fp by another) become a separate review queue rather than auto-applying. v0 assumes a single Scientist per project.

## Ground truth

The label bank is the primary ground-truth mechanism. Every labeled hunch — whether labeled live or retrospectively — feeds back into the bank and compounds the flywheel.

### Live vs retrospective labels

`labels.jsonl` files come from two sources, with different epistemic weight:

**Live labels** — generated by `hunch run` when the scientist presses g/b/s on a hunch in the moment. The scientist labeled in-context, knowing what they meant. **Gold-standard ground truth.** One labels.jsonl per actual scientific session.

**Retrospective labels** — generated by the Scientist (or a colleague) labeling hunches from a replay-driven critic-eval run. An *approximation* of what the Scientist would have said in the moment. One labels.jsonl per critic-version-replayed-on-historical-data.

Both feed the label bank, but `labeled_by` distinguishes them (e.g. `scientist_live` vs `scientist_retro`). When they conflict on the same concern, the live label outranks — the live Scientist had full context that retrospective labeling can't recover.

**Implication:** the live g/b/s feedback is the most valuable eval data we generate. Every real session produces gold labels for free, with no separate annotation step. The retrospective infra is scaffolding for evaluating critic versions on historical traces; the live labels are what the system is ultimately optimized against.

### Other label sources

The label bank can also be seeded from sources other than direct Scientist annotation — for example, hand-curated anchor cases (known-good catches for recall measurement) or automated mining of the transcript for moments where the Scientist themselves raised a concern. Technically these are just additional bank entries with a different `labeled_by` provenance (e.g. `anchor`, `mined`). The framework handles them uniformly: they participate in auto-matching the same way Scientist-labeled entries do.
