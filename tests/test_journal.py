...
def test_write_emit_produces_appendix_shape(tmp_path):
    w = HunchesWriter(hunches_path=tmp_path / "h.jsonl")
    hid = w.allocate_id()
    w.write_emit(_mk_hunch("smell-A", "desc-A"), hid, "t1", emitted_by_tick=42)
...
def test_status_change_appends_not_mutates(tmp_path):
    w = HunchesWriter(hunches_path=tmp_path / "h.jsonl")
    hid = w.allocate_id()
    w.write_emit(_mk_hunch(), hid, "t1", emitted_by_tick=1)
...
def test_read_current_hunches_folds_to_latest_status(tmp_path):
    ...
    h1 = w.allocate_id()
    h2 = w.allocate_id()
    w.write_emit(_mk_hunch("a", "aa"), h1, "t1", emitted_by_tick=1)
    w.write_emit(_mk_hunch("b", "bb"), h2, "t2", emitted_by_tick=2)
